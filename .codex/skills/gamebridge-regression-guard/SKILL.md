...
1. Locate the repository root and read `AGENTS.md` and `docs/verified-baseline.md` completely.
2. Preserve the distinction between:
   - installed-package real-device acceptance;
   - partial real-device observation;
   - automated-test coverage;
   - unverified behavior.
3. Never promote a result to real-device acceptance from unit tests, logs alone, source inspection, or an unpackaged local command.
4. Read any more specialized project Skill that matches the task when it exists.
