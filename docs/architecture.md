...
首次打开管理界面时会自动安装固定且校验过的 UMU 和默认 Proton，不把第一次
游戏启动当作运行环境下载入口。会继续发现所有已挂载 Steam 库中的 Proton，
经真机验收的单游戏兼容规则可以优先选择指定工具；其他已发现工具只有用户显式选择时才覆盖默认环境。
HoYoPlay 启动器保留独立环境，四款国际服游戏分别使用独立 Prefix，不允许 DWProton、
GE-Proton 和 Valve Proton Hotfix 在同一 Prefix 中互相覆盖。
国际服《崩坏3》只接受已经真机验收的 Valve Proton Hotfix。缺少时，独立启动器
会写入一个带时效的安装请求；Decky 前端只会接管这个已验证请求，让 Steam 下载官方工具且
不创建桌面/菜单快捷方式；启动器等待安装完成后继续当次游戏启动。不允许回退到已知无法运行 9.0 客户端的 GE-Proton。
...
外置存储不依赖固定用户名、挂载名称或 Wine 盘符。Provider 先使用上次验证成功的真实路径，
再将注册表中的相对路径匹配到所有已挂载存储根目录。启动器与游戏可以位于不同硬盘；已经有效的
Wine `G:` 映射不会被另一个存储位置覆盖，仅在映射目标已断开时自动修复。
