...
> 当前开发版本：`v0.19.0-beta.5`。这是公开前测试版，部分游戏和 SteamOS 更新后的行为仍需实机验证。
...
发布时使用项目内置脚本，它会同时检查并保留启动保护脚本的可执行权限：

```bash
python3 scripts/build_plugin_zip.py --output releases/GameBridge-dev.zip
```
